...
.PHONY: auth-status
auth-status:         ## Show current Safety authentication status
	$(SAFETY) auth status

# ── Scanning ──────────────────────────────────────────────────────────────────
.PHONY: scan
scan:                ## Scan project directory (development auth)
	$(SAFETY) scan --detailed-output

.PHONY: scan-ci
scan-ci:             ## Scan for CI/CD (requires SAFETY_API_KEY env var)
...
.PHONY: scan-fix
scan-fix:            ## Scan and auto-fix requirements.txt files
...
.PHONY: report
report: $(REPORT_DIR) ## Generate JSON + HTML vulnerability reports
	$(SAFETY) scan \
		--save-as json $(REPORT_DIR)/safety-report.json \
		--save-as html $(REPORT_DIR)/safety-report.html \
		--output screen
